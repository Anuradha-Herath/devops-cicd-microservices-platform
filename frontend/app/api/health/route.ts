  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { status: 'error', message: 'API service is not healthy' },
    ...
    console.error('Error checking API service health:', error)
    ...
    return NextResponse.json(
      { status: 'error', message: 'Failed to connect to API service' },
      { status: 503 }
    )
  }
}
